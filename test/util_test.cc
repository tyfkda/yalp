...
TEST_F(UtilTest, ListFunctions) {
  Svalue a = state_->fixnum(1);
  Svalue b = state_->fixnum(2);
  Svalue c = state_->fixnum(3);
  ...
  Svalue s3 = list(state_, state_->fixnum(1), state_->fixnum(2), state_->fixnum(3));
